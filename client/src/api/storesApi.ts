...
export async function getOffersForShop(query: OfferRequest): Promise<Offer[]> {
  const realQuery = {
    ...query,
  ...
  if (response.status < 200 || response.status >= 300) {
    throw new Error(`Expected fetch call to return an 'OK' status, but got: ${response.status}`);
  }
